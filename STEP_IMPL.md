# Retail Project step by step

# Steps
## 1. Load data into the warehouse
- Install astro and start
    ```bash
    astro dev start
    ```
- Account login airflow
    ```
    Username: admin
    Password: admin
    ```
...
- Create a service account with a name **airflow-online-retail**
    ...
    ```
    - Create connection for airflow: Airflow → Admin → Connections
        - id: gcp
        - type: Google Cloud
        - Keypath Path: **/usr/local/airflow/include/gcp/service_account.json**
    - Test the connection → Save

- Create the DAG for up file to GCS
    ```python
    # retail.py
...
- Create task for creating an empty Dataset on Bigquery 
    ```python
...
- Create the task to load the file into a BigQuery **raw_invoices** table
    ...
    gcs_to_raw = aql.load_file(
            task_id='gcs_to_raw',
            input_file=File(
                'gs://project_online_retail/raw/online_retail.csv',
                conn_id='gcp',
...
## 2. First data quality check with Soda
...
services:
  metabase:
    ...
    restart: always
```
- Go to metabase and create new question.